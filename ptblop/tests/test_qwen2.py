...
def make_model_tokenizer_unpruned_bp_config():
    model_name = "Qwen/Qwen2-0.5B"
    model_revision = "main"
    model = transformers.AutoModelForCausalLM.from_pretrained(
        model_name, revision=model_revision, torch_dtype=torch.bfloat16, trust_remote_code=True
    )
...
def check_qwen_unpruned_forward(device):
...
def test_qwen_unpruned_forward_cpu():
    check_qwen_unpruned_forward(torch.device("cpu"))


@pytest.mark.skipif(not torch.cuda.is_available(), reason="cuda not available")
def test_qwen_unpruned_forward_gpu():
    check_qwen_unpruned_forward(torch.device("cuda"))


# Test pruned attentions

def make_bp_config_with_disabled_test_attentions(bp_config0):
...
def check_qwen_disabled_attentnions(device):
    model, tokenizer, bp_config0 = make_model_tokenizer_unpruned_bp_config()
...
def test_qwen_decomposed1_cpu():
    check_qwen_disabled_attentnions(torch.device("cpu"))

@pytest.mark.skipif(not torch.cuda.is_available(), reason="cuda not available")
def test_qwen_decomposed1_gpu():
    check_qwen_disabled_attentnions(torch.device("cuda"))


# Test pruned mlp

def make_bp_config_with_disabled_test_mlps(bp_config0):
    bp_config = copy.deepcopy(bp_config0)
    ki = iter(bp_config.keys())
    _ = next(ki)
    k2 = next(ki)
...
def check_qwen_disabled_mlps(device):
    model, tokenizer, bp_config0 = make_model_tokenizer_unpruned_bp_config()
...
def test_qwen_disabled_mlps_cpu():
    check_qwen_disabled_mlps(torch.device("cpu"))

@pytest.mark.skipif(not torch.cuda.is_available(), reason="cuda not available")
def test_qwen_disabled_mlps_gpu():
