...
def validate(*, model, device, data_dir, config):
    ...
    if config.torchscript:
        ...
        torch._dynamo.reset()
        model = torch.compile(model, backend=config.torchcompile, mode=config.torchcompile_mode)
    ...
    if config.real_labels:
        real_labels = RealLabelsImagenet(dataset.filenames(basename=True), real_json=config.real_labels)
    else:
        real_labels = None
    ...
    with torch.no_grad():
        ...
        input = torch.randn((config.batch_size,) + tuple(data_config['input_size'])).to(device=device, dtype=model_dtype)
        if config.channels_last:
